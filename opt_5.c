/*
 * opt_5.c -- Kuramoto firefly sync, opt #5 only (W stored as float).
 *
 * Diff vs unoptimized: W is `float` instead of `double`. The full N*N footprint
 * is halved (4 bytes per entry instead of 8), letting twice as much of W stay
 * resident in any cache level. Math in the inner loop is still done in double:
 * each W element is promoted on load.
 *
 * rowsum is computed from the original double-precision exp() values to avoid
 * accumulating float rounding error. The inner loop's per-pair float->double
 * promotion does introduce a tiny precision drift vs the baseline -- the
 * trajectory is essentially identical at the convergence threshold (1e-4).
 */
...
int main(int argc, char **argv) {
    ...
    float   *W         = malloc((size_t)N * (size_t)N * sizeof *W);   /* opt #5 */
    ...
    for (int i = 0; i < N; ++i) {
        double sum = 0.0;
        for (int j = 0; j < N; ++j) {
            if (i == j) { W[(size_t)i * N + j] = 0.0f; continue; }
            double dx = f[i].x - f[j].x;
            double dy = f[i].y - f[j].y;
            double d  = sqrt(dx*dx + dy*dy);
            double w  = exp(-d / SIGMA);
            W[(size_t)i * N + j] = (float)w;   /* opt #5: float storage */
            sum += w;                          /* rowsum accumulated in double */
        }
        rowsum[i] = (sum > 0.0) ? sum : 1.0;
    }

    printf("# Kuramoto firefly synchronization (opt_5: W stored as float)\n");
    ...
    for (iter = 0; iter <= MAX_ITERS; ++iter) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
                if (i == j) continue;
                acc += (double)Wi[j] * sin(f[j].phase - ti);  /* promote on load */
            }
