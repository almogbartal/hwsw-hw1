/*
 * opt_1_2.c -- Kuramoto firefly sync, opts #1 and #2.
 *
 * Adds opt #2 to opt_1: accumulate sx, sy during the s[]/c[] build pass, and
 * drop the separate trig loop for the order parameter. Saves N sin + N cos
 * calls per iteration vs opt_1.
 *
 * Still uses full N*N W (no opt #3) and keeps the i==j branch in the inner
 * loop.
 */
...
int main(int argc, char **argv) {
    ...
    printf("# Kuramoto firefly synchronization (opt_1_2: sin-id + reuse s/c for r)\n");
    ...
    for (iter = 0; iter <= MAX_ITERS; ++iter) {
        /* opt #1 + #2: one s/c pass feeds both the order parameter and the
           dynamics update. */
        double sx = 0.0, sy = 0.0;
        for (int j = 0; j < N; ++j) {
            double sj = sin(f[j].phase);
            double cj = cos(f[j].phase);
            s[j] = sj;
            c[j] = cj;
            sx  += cj;
            sy  += sj;
